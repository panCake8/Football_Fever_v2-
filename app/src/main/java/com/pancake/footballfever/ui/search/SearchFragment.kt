    ...
    private fun onEvent(event: SearchUiEvent) {
        when (event) {
            is SearchUiEvent.ClickLeagueEvent -> {
                val nav =
                    SearchFragmentDirections.actionHiltSearchFragmentToLeagueStateFragment(event.league.id!!,season)
                findNavController().navigate(nav)
            }
            is SearchUiEvent.ClickTeamEvent -> {
                val nav =
                    SearchFragmentDirections.actionHiltSearchFragmentToClubFragment(event.team.id!!,event.team.id,season)
                findNavController().navigate(nav)
            }
           else->{}
        }
